...
folder_path <- "C:/Users/dimut/Downloads/cbp"
...
for (file in file_list) {
  ...
  year_suffix <- as.numeric(str_extract(file_name, "\\d{2}"))
  if(year_suffix<30) {
  ...
  if(nrows < 3000 | nrows>4000) break
  
  dt <- dt[NAICS=="------"]
  
  dt[,county:=paste0(str_pad(FIPSTATE,2,"left","0"),str_pad(FIPSCTY,3,"left","0"))]
  ...
  saveRDS(dt,paste0(folder_path,"/cbp",year,".rds"))
