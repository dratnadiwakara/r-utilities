...
dt <- readRDS("C:/data/county_large_competition_panel_Dec14_v1.rds")
setDT(dt)


dt <- dt[branches_zip_lag1>=3 & !is.na(incumbent_dep_gr_1yr) & incumbent_mkt_share_lag1>0.25]
...
setFixest_fml(
  ...
  ..vcov      = ~ county,
  ..controls = ~ + log1p(branches_zip_lag1)+county_dep_growth_t4_t1 + closer_remaining_mkt_share_lag1    +log1p(deps_lag1) #+incumbent_mkt_share_lag1+zip_dep_growth_t4_t1+log1p(deps_lag1)+closer_remaining_mkt_share_lag1,+zip_dep_growth_t4_t1
)
