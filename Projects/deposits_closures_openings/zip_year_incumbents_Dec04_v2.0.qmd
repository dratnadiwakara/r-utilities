...
```{r}
dt <- readRDS("C:/data/zip_competition_churn_panel_Dec04_v2.rds")
setDT(dt)
...
dt[,gr_3y_lag:= incumbent_dep_gr_3yr_lag]
...
vars <- c("gr_3y",
          ...
          "share_deps_closed_high_drop",
          "share_deps_closed_low_drop",
...
r[[5]] <- feols(gr_3y_cd~share_deps_closed_high_drop+share_deps_closed_low_up+..controls  | ..fixef_y_z_cbsayr,data=dt[incumbent_mkt_share_lag1>0.25 & YEAR<=2011],vcov=~ZIPBR)
r[[6]] <- feols(gr_3y_cd~share_deps_closed_high_drop+share_deps_closed_low_drop+..controls  | ..fixef_y_z_cbsayr,data=dt[incumbent_mkt_share_lag1>0.25 & YEAR>2018],vcov=~ZIPBR)
