...
```{r}
dt <- readRDS("C:/data/zip_competition_churn_panel_Nov24_v3.rds")
setDT(dt)
...
setFixest_fml(
  ...
  ..vcov      = ~ ZIPBR,
  ..fml_baseline_amt = ~share_deps_closed+log1p(branches_zip_lag1)+incumbent_mkt_share_lag1+zip_dep_growth_t4_t1+log1p(deps_lag1),
  ..fml_baseline_no = ~share_cat+log1p(branches_zip_lag1)+incumbent_mkt_share_lag1 ,
  ..fml_interaction_amt = ~share_deps_closed*sophisticated + log1p(branches_zip_lag1)+incumbent_mkt_share_lag1+zip_dep_growth_t4_t1+log1p(deps_lag1),
  ..fml_interactions_amt = ~share_deps_closed*sophisticated +share_deps_closed*above_median_income+ share_deps_closed*above_median_age + log1p(branches_zip_lag1)+incumbent_mkt_share_lag1+zip_dep_growth_t4_t1+log1p(deps_lag1)
)
...
```{r}
r <- list()
r[[1]] <- feols(gr_3y~..fml_baseline_amt | ..fixef_y_z_cbsayr,data=dt[incumbent_mkt_share_lag1>0.25 ],vcov=~ZIPBR)
r[[2]] <- feols(gr_3y~..fml_baseline_amt | ..fixef_y_z_cntyyr,data=dt[incumbent_mkt_share_lag1>0.25],vcov=~ZIPBR)
