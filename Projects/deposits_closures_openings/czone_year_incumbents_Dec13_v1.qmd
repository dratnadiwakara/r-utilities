...
```{r}
dt <- readRDS("C:/data/cz_competition_panel_Dec13_v1.rds")
setDT(dt)


dt <- dt[!is.na(incumbent_dep_gr_1yr)  & branches_cz_lag1>5]
...
# 
# 
# cz_map <- fread("c:/data/czone_county.csv")
# cz_map[, county_fips := str_pad(county, 5, "left", "0")]
# 
# county_demo_data <- merge(county_demo_data, cz_map[, .(county_fips, czone)], 
#                           by = "county_fips", all.x = TRUE)
# 
# county_demo_data <- county_demo_data[!is.na(czone)]
# 
# 
# county_demo_data[, w := total_population] 
# 
# cz_demo_data <- county_demo_data[, .(
#   # Continuous: Weighted Averages
#   median_age           = weighted.mean(median_age, w, na.rm = TRUE),
#   median_income        = weighted.mean(median_income, w, na.rm = TRUE),
#   pct_college_educated = weighted.mean(pct_college_educated, w, na.rm = TRUE),
#   capital_gain_frac    = weighted.mean(capital_gain_frac, w, na.rm = TRUE),
#   dividend_frac        = weighted.mean(dividend_frac, w, na.rm = TRUE)
# ), by = .(czone, yr)]
# 
# 
# key_chars <- c("median_income","median_age","pct_college_educated","capital_gain_frac","dividend_frac")
# 
# cz_demo_data <- data.table(cz_demo_data)
# cz_demo_data[,paste0(key_chars,"_q"):=lapply(.SD, function(x) ntile(x,2)),by=yr,.SDcols = key_chars]
# 
# cz_demo_data[,sophisticated:=ifelse(!is.na(dividend_frac)  & pct_college_educated_q==2 & (dividend_frac_q==2 | capital_gain_frac_q==2),1,
#                                            ifelse(!is.na(dividend_frac),0,NA))]
# 
# cz_demo_data[,age_bin:=ntile(median_age,2),by=yr]
# cz_demo_data[,above_median_age:=ifelse(age_bin==2,1,0)]
# cz_demo_data[,income_bin:=ntile(median_income,2),by=yr]
# cz_demo_data[,above_median_income:=ifelse(income_bin==2,1,0)]
# 
# saveRDS(cz_demo_data,"C:/data/cz_demo_data.rds")

cz_demo_data <- readRDS("C:/data/cz_demo_data.rds")


# ---------------------------------------------------------------------
# 4. Merge with Main Panel (dt)
# ---------------------------------------------------------------------
# Use czone instead of county
dt <- merge(dt, 
            cz_demo_data[, .(czone, yr, sophisticated, age_bin, median_age, median_income,
...
# Load CZ Map (assuming file exists in working dir)
cz_map <- fread("c:/data/czone_county.csv")
cz_map[, county := str_pad(county, 5, "left", "0")]

# Merge CZ info onto SOD data
sod_data <- merge(sod_data, cz_map[, .(county, czone)], by = "county", all.x = TRUE)

# Remove records that don't match a CZ (e.g., territories if not in map)
sod_data <- sod_data[!is.na(czone)]


# ---------------------------------------------------------------------
# 2. Aggregate to CZ Level
# ---------------------------------------------------------------------
cz_panel <- sod_data[, .(
...
# ---------------------------------------------------------------------
all_czs   <- unique(cz_panel$czone)
min_yr    <- min(cz_panel$YEAR)
max_yr    <- max(cz_panel$YEAR)
full_grid <- CJ(czone = all_czs, YEAR = min_yr:max_yr)

# Merge actual data onto the full grid
cz_panel_balanced <- merge(full_grid, cz_panel, by = c("czone", "YEAR"), all.x = TRUE)

# Fill NAs with 0 (No record = 0 deposits)
cz_panel_balanced[is.na(total_deps), total_deps := 0]


# ---------------------------------------------------------------------
# 4. CALCULATE LAGS AND GROWTH
# ---------------------------------------------------------------------
setorder(cz_panel_balanced, czone, YEAR)
...
cz_panel_balanced[, `:=`(
  cz_dep_growth_t4_t1 = fifelse(deps_lag4 > 0, 
...
# ---------------------------------------------------------------------
# 5. CLEANUP & MERGE TO MAIN DT
# ---------------------------------------------------------------------
# Select relevant columns to merge
# Note: merging by 'czone' and 'YEAR' now
dt <- merge(dt, 
            cz_panel_balanced[, .(czone, YEAR, total_deps, deps_lag1, deps_lag4, cz_dep_growth_t4_t1)], 
            by = c("czone", "YEAR"), 
            all.x = TRUE)

# Winsorize the new CZ growth variable
dt[, cz_dep_growth_t4_t1 := Winsorize(cz_dep_growth_t4_t1, 
                                      val = quantile(cz_dep_growth_t4_t1, probs = c(0.01, 0.99), na.rm = TRUE)), 
   by = YEAR]
...
dt[,state_year:=paste(state,YEAR)]


setFixest_fml(
  ..fixef_y_z_cbsayr     = ~ county + cbsa_yr,
  ..fixef_y_z_stateyr     = ~ czone + state_year,
  ..interactions = ~share_deps_closed*sophisticated ,
  ..vcov      = ~ czone,
  ..controls = ~ +closer_remaining_mkt_share_lag1+ log1p(branches_cz_lag1) + cz_dep_growth_t4_t1 + log1p(deps_lag1) #+incumbent_mkt_share_lag1+zip_dep_growth_t4_t1+log1p(deps_lag1)+closer_remaining_mkt_share_lag1,+zip_dep_growth_t4_t1
)
...
r[[3]] <- feols(gr_3y_cd_lag~..interactions+..controls  | ..fixef_y_z_stateyr,data=dt[YEAR<=2011])
r[[4]] <- feols(gr_3y_cd_lag~..interactions+..controls  | ..fixef_y_z_stateyr,data=dt[YEAR>=2012])
r[[5]] <- feols(gr_1y_cd~..interactions+..controls  | ..fixef_y_z_stateyr,data=dt[YEAR<=2011])
r[[6]] <- feols(gr_1y_cd~..interactions+..controls  | ..fixef_y_z_stateyr,data=dt[YEAR>=2012])
r[[7]] <- feols(gr_1y_cd_lag~..interactions+..controls  | ..fixef_y_z_stateyr,data=dt[YEAR<=2011])
r[[8]] <- feols(gr_1y_cd_lag~..interactions+..controls  | ..fixef_y_z_stateyr,data=dt[YEAR>=2012])
