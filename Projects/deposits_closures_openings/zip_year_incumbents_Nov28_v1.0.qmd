...
```{r}
dt <- readRDS("C:/data/zip_competition_churn_panel_Nov28_v1.rds")
setDT(dt)
...
dt[,high_web_int:=ifelse(weighted_web_intensity_closed>2,1,0)]
...
setFixest_fml(
  ...
  ..fml_web_inter_amt = ~share_deps_closed*weighted_web_intensity_closed +..controls,
...
r[[5]] <- feols(incumbent_dep_gr_1yr_clean_cd~share_deps_closed+..controls  | ..fixef_y_z_cbsayr,data=dt[incumbent_mkt_share_lag1>0.25 ],vcov=~ZIPBR)
r[[6]] <- feols(incumbent_dep_gr_1yr~share_deps_closed+..controls  | ..fixef_y_z_cntyyr,data=dt[incumbent_mkt_share_lag1>0.25],vcov=~ZIPBR)
