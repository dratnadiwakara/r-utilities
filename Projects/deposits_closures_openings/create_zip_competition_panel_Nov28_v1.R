...
output_path <- "C:/data/zip_competition_churn_panel_Nov28_v1.rds"
...
sod <- sod[YEAR==2021,.(RSSDID,ASSET)]
sod <- unique(sod)

web_data <- merge(web_data,sod,by.y="RSSDID",by.x="IDRSSD")
web_data[,web_visits_assets:=web_visits_2021/ASSET]
web_data[,web_visits_assets:=Winsorize(web_visits_assets, val = quantile(web_visits_assets, probs = c(0.01, 0.99), na.rm = T))]
web_data <- web_data[,.(IDRSSD,web_visits_assets)]

# Ensure naming consistency for merge (IDRSSD in file -> RSSDID in script)
setnames(web_data, old = "IDRSSD", new = "RSSDID", skip_absent = TRUE)

# Keep only necessary columns and ensure numeric
web_data <- web_data[, .(RSSDID, web_visits_assets)]
web_data[, web_visits_assets := as.numeric(web_visits_assets)]
...
closure_opening_data <- merge(closure_opening_data, web_data, 
...
closure_opening_data[is.na(web_visits_assets), web_visits_assets := 0] 
...
closer_agg_final <- relevant_branches[, .(
  ...
  closer_closed_web_weighted_vol = sum_na_safe(fifelse(closed == 1L, dep_lag1_aligned * web_visits_assets, NA_real_)),
...
cols_to_zero <- c("closer_closed_vol_lag1", "closer_opened_vol_curr", 
                  ...
                  "closer_closed_web_weighted_vol") # Added new column to zero fill
...
final_panel[, `:=`(
  ...
  weighted_web_intensity_closed = fifelse(closer_closed_vol_lag1 > 0,
                                          closer_closed_web_weighted_vol / closer_closed_vol_lag1,
                                          0),
