...
```{r}
dt <- readRDS("C:/data/zip_competition_churn_panel_refined_6.rds")
setDT(dt)
...
```{r}
dt[, small_shock := total_closed_vol_zip_lag1 < 10000]


r <- list()
r[[1]] <- feols(incumbent_growth_share_3yr~share_deps_closed+log1p(branches_zip_lag1)+small_shock|ZIPBR+cbsa_yr,data=dt)
r[[2]] <- feols(incumbent_growth_share_3yr~share_deps_closed+log1p(branches_zip_lag1)+log1p(total_closed_vol_zip_lag1) |ZIPBR+cbsa_yr,data=dt)
r[[3]] <- feols(incumbent_growth_share_3yr~share_deps_closed+log1p(branches_zip_lag1)|ZIPBR+cbsa_yr,data=dt[total_closed_vol_zip_lag1<10000])
...
r <- list()
r[[1]] <- feols(incumbent_growth_share_3yr~share_deps_closed*sophisticated_ed_sm+log1p(branches_zip_lag1)+small_shock|ZIPBR+cbsa_yr,data=dt)
r[[2]] <- feols(incumbent_growth_share_3yr~share_deps_closed*sophisticated_ed_sm+log1p(branches_zip_lag1)+log1p(total_closed_vol_zip_lag1) |ZIPBR+cbsa_yr,data=dt)
r[[3]] <- feols(incumbent_growth_share_3yr~share_deps_closed*sophisticated_ed_sm+log1p(branches_zip_lag1)|ZIPBR+cbsa_yr,data=dt[total_closed_vol_zip_lag1<10000])

etable(r)
```

```{r}
# Create 4 buckets of Closure Size
dt[, vol_quartile := ntile(total_closed_vol_zip_lag1,4),by=YEAR]

# Run the model interacting with these quartiles
t <- feols(incumbent_growth_share_3yr ~ i(vol_quartile,share_deps_closed) | ZIPBR + county_yr, data = dt)
etable(t)
```
