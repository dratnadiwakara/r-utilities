...
pop_path    <- "C:/data/zip_irs_acs.rds" # Path to the population file we created
output_path <- "C:/data/zip_competition_churn_panel_pop_denom.rds"

# ---------------------------------------------------------------------
# 0. PREPARE POPULATION DATA (NEW SECTION)
# ---------------------------------------------------------------------
# Load the panel created in the previous step
pop_panel <- readRDS(pop_path)
pop_panel <- pop_panel[!is.na(year) & !is.na(population)]

# Ensure we only keep necessary columns
pop_panel <- pop_panel[, .(zipcode, year, population)]

# Convert zipcode to integer to match the FDIC "ZIPBR" format
pop_panel[, ZIPBR := zipcode]
setnames(pop_panel,"year","YEAR")

# Create Reference Tables for your Fill Logic
pop_2013 <- pop_panel[YEAR == 2013, .(ZIPBR, pop_2013 = population)]
pop_2022 <- pop_panel[YEAR == 2022, .(ZIPBR, pop_2022 = population)]

# Create a full grid of Zips and Years (2000 to 2025) to ensure we cover all FDIC years
all_zips  <- unique(pop_panel$ZIPBR)
full_grid <- CJ(ZIPBR = all_zips, YEAR = 2000:2025)

# Merge references onto the full grid
full_grid <- merge(full_grid, pop_panel[, .(ZIPBR, YEAR, population_actual = population)], 
                   by = c("ZIPBR", "YEAR"), all.x = TRUE)
full_grid <- merge(full_grid, pop_2013, by = "ZIPBR", all.x = TRUE)
full_grid <- merge(full_grid, pop_2022, by = "ZIPBR", all.x = TRUE)

# --- APPLY THE DATE LOGIC ---
full_grid[, population_filled := fifelse(YEAR < 2013, pop_2013,
                                         fifelse(YEAR > 2022, pop_2022, population_actual))]

# Cleanup: Keep only final population variable
zip_population_final <- full_grid[!is.na(population_filled), .(ZIPBR, YEAR, population_filled)]
...
closers_agg <- bank_zip_panel[bank_type %in% c("CLOSER"), .(
...
# ---------------------------------------------------------------------
# First merge incumbents with zip stats
...
# --- NEW MERGE: Add the Population Data ---
final_zip_panel <- merge(final_zip_panel,
                         zip_population_final[, .(ZIPBR, YEAR, population_filled)],
                         by = c("ZIPBR", "YEAR"),
                         all.x = TRUE)
...
final_zip_panel[, `:=`(
  ...
  # MODIFIED: Denominator is now Population at time t (population_filled)
  share_deps_closed = fifelse(total_deps_zip_lag1 > 0 & !is.na(total_deps_zip_lag1), 
                              total_closed_vol_zip_lag1 / total_deps_zip_lag1, 0),
  
  share_deps_closed_pp = fifelse(population_filled > 0 & !is.na(population_filled), 
                              total_closed_vol_zip_lag1 / population_filled, 0),
  ...
  incumbent_growth_share_3yr_pp = fifelse(population_filled > 0,
                                       (incumbent_deps_lead3 - incumbent_deps_curr) / population_filled,
                                       NA_real_),
  incumbent_growth_share_1yr_pp = fifelse(population_filled > 0,
                                       (incumbent_deps_lead1 - incumbent_deps_curr) / population_filled,
                                       NA_real_),
