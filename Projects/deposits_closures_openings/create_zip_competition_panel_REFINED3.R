...
output_path <- "C:/data/zip_competition_churn_panel_refined_4.rds"
...
zip_market_stats <- closure_opening_data[, .(
  ...
  total_closed_vol_zip_lag1 = sum_na_safe(fifelse(closed == 1L, DEPSUMBR, NA_real_))
), by = .(ZIPBR, YEAR)]
...
final_zip_panel[, `:=`(
  ...
  incumbent_growth_share_1yr = fifelse(total_deps_zip_lag1 > 0,
                                       (incumbent_deps_curr - incumbent_deps_lag1) / total_deps_zip_lag1,
                                       NA_real_),
  ...
  incumbent_growth_share_1yr_alt = fifelse(total_deps_zip_lag1 > 0,
                                           (incumbent_deps_curr - incumbent_deps_curr) / incumbent_deps_curr,
                                           NA_real_)
)]
